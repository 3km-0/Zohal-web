...
DEFAULT_SUPABASE_URL_FILE="${WORKSPACE_ROOT}/zohal-core/supabase/credentials.md"
...
if [[ -z "${RESOLVED_SUPABASE_URL}" && -f "${DEFAULT_SUPABASE_URL_FILE}" ]]; then
  RESOLVED_SUPABASE_URL="$(awk '/^URL:/ { print $2; exit }' "${DEFAULT_SUPABASE_URL_FILE}")"
fi
RESOLVED_SUPABASE_ANON_KEY="${SUPABASE_ANON_KEY:-${SUPABASE_PUBLISHABLE_KEY:-}}"
if [[ -z "${RESOLVED_SUPABASE_ANON_KEY}" && -f "${DEFAULT_SUPABASE_URL_FILE}" ]]; then
  RESOLVED_SUPABASE_ANON_KEY="$(awk -F': ' '/^Anon Key:/ { print $2; exit }' "${DEFAULT_SUPABASE_URL_FILE}")"
fi
...
if [[ -n "${RESOLVED_SUPABASE_URL}" ]]; then
  DEFAULT_ENV_VARS="${DEFAULT_ENV_VARS},SUPABASE_URL=${RESOLVED_SUPABASE_URL}"
fi
if [[ -n "${RESOLVED_SUPABASE_ANON_KEY}" ]]; then
  DEFAULT_ENV_VARS="${DEFAULT_ENV_VARS},SUPABASE_ANON_KEY=${RESOLVED_SUPABASE_ANON_KEY}"
fi
